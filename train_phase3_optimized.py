...
CHEXPERT_TRAIN_CSV = os.path.join(CHEXPERT_ROOT, "train.csv")
CHEXPERT_VALID_CSV = os.path.join(CHEXPERT_ROOT, "valid.csv")
...
class NIHDataset(Dataset):
    def __init__(self, csv_path, image_dir, transform=None, selected_labels=None, sample_size=None, patient_level_split=True):
        ...
        self.nih_to_selected = {
            ...
            'Edema': 'Edema',
            'Effusion': 'Pleural Effusion'  # CRITICAL: NIH uses "Effusion" not "Pleural Effusion"
        }
        ...
        # CRITICAL: Precompute label matrix for fast access + validation
        ...
        # NIH exact disease names (as they appear in CSV)
        ...
        self.labels[:, LABEL_TO_IDX['Pleural Effusion']] = has('Effusion').astype(np.float32).values
...
class PneumoniaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            # Safe label alignment using LABEL_TO_IDX
        ...
        if AMP_ENABLED and scaler:
            ...
            if ((batch_idx + 1) % ACCUMULATION_STEPS == 0) or ((batch_idx + 1) == num_batches):
                ...
                if scaler.get_scale() >= scale_before:
                    scheduler.step(epoch - 1 + (batch_idx + 1) / num_batches)
        else:
            outputs = model(images)
            if labels_b is not None:
                loss = mixup_criterion(criterion, outputs, labels_a, labels_b, lam)
            ...
            if ((batch_idx + 1) % ACCUMULATION_STEPS == 0) or ((batch_idx + 1) == num_batches):
                ...
                # Step scheduler per batch with fractional epoch for smooth warm restarts
                scheduler.step(epoch - 1 + (batch_idx + 1) / num_batches)
...
def main():
    try:
        ...
        chexpert_train = CheXpertDataset(
            CHEXPERT_TRAIN_CSV, CHEXPERT_ROOT, train_transform, 
            SELECTED_LABELS, CHEXPERT_SAMPLE_SIZE
        )
        chexpert_valid = CheXpertDataset(
            CHEXPERT_VALID_CSV, CHEXPERT_ROOT, val_transform, SELECTED_LABELS
        )
        
        # NIH - Use persistent fixed split files to prevent data leakage on resume
        nih_df = pd.read_csv(NIH_CSV_PATH)
        
        # Define permanent split files in CHECKPOINT_DIR to ensure continuity across resumes
        fixed_train_csv = os.path.join(CHECKPOINT_DIR, "nih_fixed_train.csv")
        fixed_val_csv = os.path.join(CHECKPOINT_DIR, "nih_fixed_val.csv")
        
        if os.path.exists(fixed_train_csv) and os.path.exists(fixed_val_csv):
            print(f"Loading existing fixed NIH splits from {fixed_train_csv}...")
            nih_train_df = pd.read_csv(fixed_train_csv)
            nih_val_df = pd.read_csv(fixed_val_csv)
        else:
            print("Creating new patient-level NIH splits...")
            # Robust path resolution for NIH official split file
            nih_root = os.path.dirname(NIH_CSV_PATH)  # .../datasets/nih
            
            candidates = [
                os.path.join(nih_root, "train_val_list.txt"),
                os.path.join(nih_root, "..", "train_val_list.txt"),
            ]
            
            train_val_list = next((p for p in candidates if os.path.exists(p)), None)
            
            if train_val_list is not None:
                print(f"Found NIH official split: {train_val_list}")
                with open(train_val_list) as f:
                    train_val_images = set(line.strip() for line in f)
                nih_train_df = nih_df[nih_df['Image Index'].isin(train_val_images)]
                nih_val_df = nih_df[~nih_df['Image Index'].isin(train_val_images)]
            else:
                print("NIH official split not found, creating patient-level random split (80/20)")
                patients = nih_df['Patient ID'].unique()
                train_patients = set(np.random.choice(patients, int(0.8 * len(patients)), replace=False))
                nih_train_df = nih_df[nih_df['Patient ID'].isin(train_patients)]
                nih_val_df = nih_df[~nih_df['Patient ID'].isin(train_patients)]
            
            # SAVE IMMEDIATELY to prevent leakage on future runs
            nih_train_df.to_csv(fixed_train_csv, index=False)
            nih_val_df.to_csv(fixed_val_csv, index=False)
            print(f"NIH splits saved to {fixed_train_csv} and {fixed_val_csv}. Future runs will use these exact files.")
        ...
        # Save to temp for Dataset class (will use fixed splits as source)
        ...
        # Pneumonia
        pneumonia_train = PneumoniaDataset(PNEUMONIA_ROOT, 'train', train_transform)
        pneumonia_valid = PneumoniaDataset(PNEUMONIA_ROOT, 'test', val_transform)
        ...
        eff_idx = LABEL_TO_IDX['Pleural Effusion']
        ...
        for epoch in range(start_epoch, EPOCHS + 1):
            ...
            # Train
            train_loss = train_one_epoch(model, train_loader, criterion, optimizer, scaler, scheduler, epoch)
            ...
            if epoch % VALIDATE_EVERY_N_EPOCHS == 0 or epoch == EPOCHS:
                ...
                nih_eff_auc = nih_per_class.get('Pleural Effusion')
                ...
                # Core datasets (NIH + CheXpert) must achieve 85%+ on ALL 5 diseases
                min_auc_core = compute_min_auc([nih_per_class, chexpert_per_class])
                
                # Pneumonia dataset only has Consolidation labels
                pneu_cons = pneumonia_per_class.get("Consolidation")
                pneu_cons_auc = float(pneu_cons) if pneu_cons is not None else 0.0
                
                # Global min AUC for checkpoint criterion: must achieve threshold on BOTH
                # - All 5 diseases in core datasets (NIH+CheXpert)
                # - Consolidation in Pneumonia dataset
                min_auc = min(min_auc_core, pneu_cons_auc)
                
                # CRITICAL: Monitor key metrics with enhanced warnings
                print(f"\n{'='*70}")
                print("CRITICAL METRIC TRACKING")
                print(f"{'='*70}")
                print(f"NIH Pleural Effusion AUC:      {nih_eff_auc:.4f}  <- Historically challenging")
                print(f"Pneumonia Consolidation AUC:   {pneu_cons_auc:.4f}  <- Binary classification")
                print(f"Min AUC (Core 5 diseases):     {min_auc_core:.4f}  <- NIH+CheXpert all 5")
                print(f"Min AUC (Global):              {min_auc:.4f}  <- CHECKPOINT CRITERION")
                
                if epoch >= 10 and nih_eff_auc < 0.60:
                    print("\n⚠️  WARNING: NIH Effusion AUC still low at epoch 10+")
                    print("   Expected >60% by epoch 10. Check if labels are being learned.")
                if epoch >= 10 and min_auc_core < 0.65:
                    print("\n⚠️  WARNING: Core min AUC still low at epoch 10+")
                    print("   Expected >65% by epoch 10 for all 5 diseases.")
                if epoch >= 10 and pneu_cons_auc < 0.75:
                    print("\n⚠️  WARNING: Pneumonia Consolidation AUC low at epoch 10+")
                ...
                epoch_data["validation"] = {
                    ...
                    "pneumonia_consolidation_auc": float(pneu_cons_auc),  # Track separately
                    "mean_auc": float(mean_auc),
                    "min_auc_core": float(min_auc_core),  # Track core 5 diseases
                ...
                with open(LOG_FILE, 'a') as f:
                    f.write(f"Epoch {epoch}/{EPOCHS} | Train Loss: {train_loss:.4f} | "
                ...
                print(f"{'='*70}")
                print(f"SUMMARY - Epoch {epoch}/{EPOCHS}")
                print(f"{'='*70}")
